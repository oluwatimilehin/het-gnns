...
class Transformer(nn.Module):
    ...
    def __init__(self, n_channels, num_heads=1, att_drop=0.0, act="none"):
    ...
    def forward(self, x, mask=None):
        ...
        beta = F.softmax(
            self.act(f @ g / math.sqrt(f.size(-1))), dim=-1
        )  # [B, H, M, M(normalized)]
...
class SeHGNN(nn.Module):
    ...
    def __init__(
        ...
        self.num_channels = num_channels = len(self.feat_keys) + len(
        ...
        self.semantic_fusion = Transformer(
            hidden, num_heads=num_heads, att_drop=att_drop, act=act
        )
        ...
        if self.dataset not in ["IMDB", "Freebase"]:
            ...
            self.task_mlp = nn.ModuleList(
                [nn.Sequential(nn.PReLU(), nn.Dropout(dropout))]
                + [
                    nn.Sequential(
                        nn.Linear(hidden, hidden),
                        nn.BatchNorm1d(hidden, affine=False),
    ...
    def forward(self, batch, feature_dict, label_dict={}, mask=None):
        if isinstance(feature_dict[self.tgt_type], torch.Tensor):
            features = {
                k: self.input_drop(x @ self.embeding[k])
            ...
            features = {
                k: self.input_drop(x @ self.embeding[k[-1]])
                for k, x in feature_dict.items()
            }
        else:
            assert 0
        ...
        labels = {
            k: self.input_drop(x @ self.labels_embeding[k])
            for k, x in label_dict.items()
        }

        x = [features[k] for k in self.feat_keys] + [
            labels[k] for k in self.label_feat_keys
        ]
